...
findSubstrings' s ( Node ( (a, t) : ts ) )
  | isPrefix s a = getIndices (Node [(a, t)]) ++ findSubstrings' s (Node ts)
  | isPrefix a s = findSubstrings' (removePrefix a s) t ++ findSubstrings' s (Node ts)
  | otherwise    = findSubstrings' s (Node ts)

------------------------------------------------------

-- t1
-- Node [("banana",Leaf 0),("a",Node [("na",Node [("na",Leaf 1),("",Leaf 3)]),("",Leaf 5)]),("na",Node [("na",Leaf 2),("",Leaf 4)])]
--
-- Node [("banana",Leaf 0),("a",Node [("",Leaf 5),("na",Node [("",Leaf 3),("na",Leaf 1)])]),("na",Node [("",Leaf 4),("na",Leaf 2)])]
...
longestRepeatedSubstring (Node [])
  = []
longestRepeatedSubstring ( Node ( (a, t) : ts ) )
  | repeated t = longest (a ++ longestRepeatedSubstring t, longestRepeatedSubstring (Node ts))
  | otherwise  = longestRepeatedSubstring (Node ts)
  where
    repeated :: SuffixTree -> Bool
    repeated t
      | (Node (_:_:_)) <- t = True   -- Node has at least 2 subtrees
      | otherwise           = False
