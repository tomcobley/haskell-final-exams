...
skipSpace str
  | (c:cs) <- str, isSpace c = skipSpace cs 
  | otherwise                = str  

getAttribute :: String -> XML -> String
getAttribute attName xml
  | Element _ atts _ <- xml = fromMaybe "" (lookup attName atts)
  | otherwise               = ""

getChildren :: String -> XML -> [XML]
getChildren name xml
  | Element _ _ children <- xml = filter (((==) name) . getName) children 
  | otherwise                   = [] 
  where 
    getName :: XML -> String 
    getName xml 
...
getChild name xml
  | (child:_) <- getChildren name xml = child 
  | otherwise                         = Text ""

addChild :: XML -> XML -> XML
-- Pre: the second argument is an Element
addChild child (Element name atts children)
  = Element name atts (children ++ [child])
...
getValue xml
  ...
  where 
    getValue' :: XML -> String 
    getValue' xml
      | Element _ _ children <- xml = concatMap getValue' children
      | Text str             <- xml = str 
      | otherwise                   = ""
...
parseAttributes text
  | '>' : text' <- skipSpace (text) = ([], text')
  | otherwise                       = ((name, value) : atts, textRemaining) 
  where 
    (name, text')           = parseName (skipSpace text)
    (value, text'')         = parseValue (skipSpace text')
    (atts, textRemaining)   = parseAttributes text''

parseValue :: String -> (String, String)
parseValue text 
  = (value, text')
  where 
    (value, _:text') = break ('\"' ==) (tail (skipSpace (tail text)))
...
parse' "" ( (Element _ _ (c:cs) ) :es)
  = c 

parse' ('<':'/':text) stack 
  = parse' text' (popAndAdd stack)
  where 
    (_, _:text') = break (== '>') text

parse' ('<':text) stack 
  = parse' text'' ((Element name attributes []) : stack)
  where 
    (name, text') = parseName text
    (attributes, text'') = parseAttributes text'  

parse' text (e:es) 
  = parse' text' ( (addChild (Text content) e) : es )
  where 
...
expandXSL' xml xsl@(Element "value-of" _ _)
  | i:is <- find (getAttribute "select" xsl) xml = [getValue i]
  | otherwise                                    = [Text ""]
expandXSL' xml xsl@(Element "for-each" _ children)
  = concat [ expandXSL' xml' child 
    | xml' <- find (getAttribute "select" xsl) xml, child <- children ]
expandXSL' xml (Element name atts children)
  = [Element name atts (concatMap (expandXSL' xml) children)]
expandXSL' xml xsl 
  = [xsl]

find :: String -> Context -> [XML]
find xpath context
  | ""                          <- xpath  = [context] 
  | '@' : attName               <- xpath  = [Text (getAttribute attName context)]
  | "."       <- step,   []     <- xpath' = [context] 
  | "."       <- step,   _:path <- xpath' = find path context
  | childName <- step,   []     <- xpath' = getChildren childName context
  | childName <- step,   _:path <- xpath' = concatMap (find path) (getChildren childName context)
  where 
