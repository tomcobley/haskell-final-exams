...
updateVar (id, val) state
  | Nothing             <- lookup id state = (id, (Local, val)) : state
  | Just old@(scope, _) <- lookup id state = (id, (scope, val)) : (delete (id, old) state) 
...
applyOp op (I x) (I y)
  | op == Add   = I (x + y)
  | op == Mul   = I (x * y)
  | op == Less  = I (if x < y then 1 else 0)
  | op == Equal = I (if x == y then 1 else 0)
...
eval e fs state
  | Const c        <- e = c 
  | Var id         <- e = getValue id state
  | OpApp op e1 e2 <- e = applyOp op (eval e1 fs state) (eval e2 fs state)
eval (Cond condition e1 e2) fs state
  | eval condition fs state == (I 1) = eval e1 fs state
  | otherwise                        = eval e2 fs state
eval (FunApp id es) fs state
  = eval e fs (bindings ++ state)
  where
    (as, e)   = lookUp id fs
    vs        = evalArgs es fs state 
    bindings  = bindArgs as vs 
...
executeStatement (Assign id e) fs ps state
  = updateVar (id, eval e fs state) state
executeStatement (AssignA id e1 e2) fs ps state
  -- assuming e1 is the index and e2 the value to assign
  = updateVar (id, assignArray (getValue id state) (eval e1 fs state) 
                   (eval e2 fs state) ) state
executeStatement (If p b1 b2) fs ps state
  = if eval p fs state == I 1 then executeBlock b1 fs ps state 
                              else executeBlock b2 fs ps state
executeStatement (While p b) fs ps state
  = if eval p fs state == I 1 
    then executeStatement (While p b) fs ps (executeBlock b fs ps state) 
    else state
executeStatement (Return e) fs ps state   
  = ("$res", (Local, eval e fs state)) : state
executeStatement (Call resultId procId es) fs ps state
  | resultId == "" = resultState
  | otherwise      = updateVar (resultId, getValue "$res" s') resultState
  where 
    (as, block)       = lookUp procId ps
    (globals, locals) = (getGlobals state, getLocals state)
    vs                = evalArgs es fs state 
    s'                = executeBlock block fs ps ((bindArgs as vs) ++ globals)
    resultState       = locals ++ (getGlobals s')

executeBlock :: Block -> [FunDef] -> [ProcDef] -> State -> State
-- Pre: All code blocks and associated statements are well formed
executeBlock block fs ps state
  | s:ss <- block = executeBlock ss fs ps (executeStatement s fs ps state)
  | otherwise     = state
