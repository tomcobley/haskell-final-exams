...
execStatement (Assign id exp) s
  = update (id, eval exp s) s 
execStatement (If exp b1 b2) s
  | eval exp s == 1 = execBlock b1 s 
  | otherwise       = execBlock b2 s
execStatement (DoWhile b exp) s
  | eval exp s' == 1 = execStatement (DoWhile b exp) s'
  | otherwise        = s'
  where 
    s' = (execBlock b s)
...
execBlock [] s 
  = s
execBlock (statement:block) s
  = execBlock block (execStatement statement s) 
...
foldConst exp
  | Phi (Const c) (Const c')      <- exp, c == c' = (Const c)
  | Apply op (Const c) (Const c') <- exp          = (Const (apply op c c'))
  | Apply Add (Var v) (Const 0)   <- exp          = (Var v)
  | Apply Add (Const 0) (Var v)   <- exp          = (Var v)
  | otherwise                                     = exp

sub :: Id -> Int -> Exp -> Exp
-- Pre: the expression is in SSA form
sub id n exp
  | Var id'         <- exp, id == id' = foldConst (Const n)
  | Apply op e1 e2  <- exp            = foldConst (Apply op (sub id n e1) (sub id n e2))
  | Phi e1 e2       <- exp            = foldConst (Phi (sub id n e1) (sub id n e2))
  | otherwise                         = foldConst (exp)
...
scan id n ( (If e b1 b2) : sts )
  = (w1' ++ w2' ++ worklist', (If e' b1' b2') : block' )
  where 
    (w1', b1') = scan id n b1
    (w2', b2') = scan id n b2
    (worklist', block') = scan id n sts
...
propagateConstants b
  ...
  where 
    -- $INVALID will not exist in the block so no substitutions will occur
    (initialWorklist, _) = scan "$INVALID" 0 b

    propagateConstants' :: Block -> Worklist -> Block 
    propagateConstants' b []
      = b
    propagateConstants' block ((v, c) : worklist)
      = propagateConstants' block' (worklist ++ worklist')
      where 
        (worklist', block') = scan v c block
...
unPhi [] 
  = []
unPhi (st:sts) 
  | If e b1 b2  <- st, (b1', b2')   <- getPhis sts = If e (unPhi (b1 ++ b1')) (unPhi (b2 ++ b2')) : unPhi sts
  | DoWhile b e <- st, (b1'', b2'') <- getPhis b   = b1'' ++ [(DoWhile (unPhi (b ++ b2'')) e)] ++ unPhi sts
  | Assign _ (Phi _ _)              <- st          = unPhi sts
  | otherwise                                      = st : unPhi sts
  where 
    getPhis :: Block -> (Block, Block) 
    getPhis [] 
      = ([], [])
    getPhis (st:sts)
      | (Assign id (Phi e1 e2)) <- st = ( (Assign id e1) : st1s, (Assign id e2) : st2s )
      | otherwise                     = ([], [])
      where 
        (st1s, st2s) = getPhis sts
